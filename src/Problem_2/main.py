...
def tf_idf(docs, queries, tokenizer):
    ...
    dictionary: list
        sorted dictionary
    """
    space = [' ' for i in range(32)]
    s = ''.join(space)
    trans = str.maketrans(string.punctuation,s)
    # nltk.download('stopwords', quiet=True, raise_on_error=True)
    # stop_words = set(nltk.corpus.stopwords.words('english'))
    # tokenized_stop_words = nltk.word_tokenize(' '.join(nltk.corpus.stopwords.words('english')))
    # print(tokenized_stop_words)
    processed_docs = [d.lower().translate(trans) for d in docs]
    # tfidf = TfidfVectorizer(stop_words=tokenized_stop_words, tokenizer=tokenizer)
...
if __name__ == '__main__':
    ...
    # print(np.argsort(-sim_matrix),np.argsort(-rf_sim_matrix))
    if args.verbose:
    ...
    # print(tfidf_model.inverse_transform())
    # words = []
    ...
    rf_sim_matrix = relevance_feedback_exp(vec_docs, d, sim_matrix, tfidf_model)
    if args.verbose:
        for i in range(rf_sim_matrix.shape[1]):
            ranked_documents = np.argsort(-rf_sim_matrix[:, i])
            print ('Query:', i+1, 'Top relevant 10 documents:', ranked_documents[:10] + 1)
